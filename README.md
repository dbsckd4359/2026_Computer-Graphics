...
## 흐름
`어린 시절 회상` 이라는 테마로 배경은 놀이터이며, fog를 사용한 연출로 시작을 하며 특정 위치에 진입하면 기본적인 테마인 `Morning`으로 시작합니다. 이후 유틸리티 영역인 타이머의 제한시간동안 기구와 분위기를 직관적으로 확인할 수 있습니다. 그외의 테마는 `Sunset`,`Night`모드가 존재합니다.
